...
- Реализованы и протестированы ключевые улучшения:
  1. **Pre-Speech кольцевой буфер (300 мс @ 16kHz)**:
     - Устранено срезание глухих и взрывных согласных («п», «т», «к», «с», «ш») в начале речи.
  2. **Чистый VAD автостоп по паузе в речи (без лимитов длины реплики)**:
     - Убран принудительный 15-секундный таймер. Запись останавливается строго после наступления непрерывной тишины на время, заданное слайдером VAD (3–15 сек).
  3. **Сужение длины пилюли записи и тумблер Live Speech Preview**:
     - Длина окна записи с текстом сужена до 320px.
     - В «Настройки → Интерфейс» добавлен тумблер «Живой предпросмотр речи». При отключении окно записи сворачивается в компактную пилюлю (220px) с живым эквалайзером волн (`WaveformVisualizer`) и аккуратным бейджем обработки.
  4. **Backpressure в фоновых воркерах живого стриминга**:
     - Неблокирующий `is_inflight` флаг предотвращает накопление очередей interim-запросов при задержках сети.
  6. **Универсальный Engine-Agnostic Fallback & Resilience Pipeline**:
     - Внедрён детектор плотности речи (`Stream Completeness Verification`): если стриминг замер на середине длинной фразы, система автоматически выполняет батч-STT на полном WAV-буфере.
     - Добавлен автоматический повтор (Retry 1.2s) при HTTP 429/503/500 ошибках LLM-форматтеров (GigaChat, Gemini, Groq, DeepSeek, Qwen).
     - Лимит токенов `max_tokens` увеличен до 4096 (запас на ~20 минут непрерывного монолога).
     - Гарантированное сохранение сырого STT (`final_text`), если AI-форматирование недоступно.
- Все проверки пройдены: `cargo check`, `cargo clippy -- -D warnings` (0 warnings), `cargo fmt --check`, `cargo test` (98 pass), `npx vitest run` (67 pass), `npx eslint` (0 errors).
