...
- Реализованы и протестированы ключевые улучшения:
  ...
  5. **Четкое разделение звуков заминки и эмоциональных междометий в LLM**:
     - `REFINEMENT_SYSTEM_PROMPT` и `FORMAT_STYLE_LIGHT/DEEP` обновлены: удаляются хезитации («эээ», «нуу», «хмм», заикания, фальстарты) и СОХРАНЯЮТСЯ эмоциональные междометия («М-м, как вкусно!», «Ого!», «Эх!», «Увы!»).
