...
Дата: 2026-08-31
Кто/что обновило: Агент (Кастомизация моделей AI, VAD автостоп, 100% дословность, Live-стриминг)
...
- Реализованы и протестированы ключевые фичи:
  1. **Кастомизация моделей AI**: выбор пресетов и ввод кастомных названий моделей для Groq, Gemini, DeepSeek, Qwen (`qwen3.7-flash`), GigaChat.
  2. **100% дословность AI-форматирования**: промпты откалиброваны строго на пунктуацию и грамматику без подмены слов.
  3. **Адаптивный VAD**: автостоп по паузе речи (3–15 сек) с динамическим отслеживанием `noise_floor`, защищенный от фонового шума кулера/комнаты.
  4. **Универсальный Live-стриминг фраз**: промежуточное отображение текста на экране во время речи для всех движков STT.
  5. **Мгновенная финализация**: приоритет уже настримленного текста — исключена потеря последней фразы и устранена задержка при повторном STT.
