...
Дата: 2026-09-01
Кто/что обновило: Агент (Оптимизация Real-Time стриминга для всех STT провайдеров и сохранение 100% качества)
...
- Реализованы и протестированы ключевые фичи:
  1. **Нативный Real-Time WebSocket стриминг для Deepgram**:
     - Прямое WebSocket подключение (`wss://api.deepgram.com/v1/listen`) с отправкой 16-битных PCM аудио-чанков каждые 120 мс.
     - Мгновенный вывод промежуточных распознанных фраз (`interim_results`) с задержкой ~150–250 мс.
     - Автоматический fallback на ускоренный HTTP polling при сбоях сокета.
  2. **Сверхбыстрый мульти-провайдерный Live-стриминг (Groq, Gemini, Whisper)**:
     - Начальная задержка уменьшена с 1500 мс до 350 мс.
     - Шаг опроса снижен с 1600 мс до 400 мс.
     - Поддержка индивидуальных API-ключей (Groq, Gemini) без обязательной привязки к Groq.
  3. **Гарантия 100% качества и отсутствие потери слов (Final Pass)**:
     - Устранен ложный fast-path, который обрезал концовки фраз из-за использования устаревшего снимка превью.
     - Финальный проход всегда захватывает 100% аудио-буфера с аудио-хвостом (300–500 мс) и применяет выбранный STT + AI-форматирование.
     - Превью-текст используется как безопасный fallback только при сбое финального запроса.
  4. **Индикация копирования и оптимизация истории**: мемоизация `HistoryCard`, мгновенный поиск, Escape-навигация.
- Все проверки пройдены: `cargo check`, `cargo clippy -- -D warnings` (0 warnings), `cargo fmt --check`, `cargo test` (93 pass), `bun run lint` (0 errors), `bun run test` (62 pass), `bun tauri build` (успешная сборка dmg и app).
...
## Что сделано
- **Сессия 15 (Оптимизация Real-Time стриминга для всех STT провайдеров и сохранение 100% качества):**
  - **Deepgram Native WebSocket Streaming**:
    - Интеграция `tokio-tungstenite` с передачей `Message::Binary` (16kHz linear16 PCM) каждые 120 мс во время записи.
    - Парсинг live-событий Deepgram и эмит `interim-transcription` на фронтенд.
    - Реализована функция `samples_to_i16_pcm` в `utils.rs` с модульным тестом.
  - **Мульти-провайдерный стриминг**:
    - Оптимизирован `spawn_interim_stream_worker` в `ai_provider.rs`, `whisper/recording.rs`, `deepgram.rs`.
    - Время старта снижено до 350 мс, интервал обновления до 400 мс.
    - Добавлена независимая поддержка ключей Gemini / Groq.
  - **Гарантия дословности и качества при стопе**:
    - В `commands/audio.rs` убран fast-path bypass, заменен на гарантированный Full STT Pass + AI formatting по полному буферу с fallback на live text в случае сбоя сети.
    - В `useRecording.ts` сохраняется отформатированный результат без перезаписи более длинным сырым превью.
  - **Тесты и сборка**: 93 Rust unit tests pass, 62 Vitest frontend tests pass, clippy 0 warnings, eslint 0 warnings, `bun tauri build` OK.
- **Сессия 14 (Оптимизация производительности окна истории и визуальная индикация копирования):**
