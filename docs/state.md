...
## Что сейчас в работе
- Завершена реализация непрерывного живого стриминга без задержек по запросу «редактируем стриминг и логику interim-воркера»:
  1. **Снятие блокировки по паузе в interim-воркере**: удалено условие ожидания пауз `if is_paused || is_hard_cutoff` для отправки текста в UI. Промежуточный драфт эмитится непрерывно во время речи.
  2. **Гибридный стриминг (Groq priority + Local Whisper fallback)**:
     - При наличии ключа Groq: быстрый стример обрабатывает растущий буфер каждые 500–600 мс (первый чанк через 450 мс), обеспечивая мгновенный старт и плавность без рывков.
     - При отсутствии ключа (чистый офлайн): локальный Whisper обрабатывает скользящее окно (2.5–3.0 с) каждые 800 мс без ожидания пауз.
  3. **Сохранение контекста и защита от стирания экрана**: для офлайн-режима текст формируется через `safe_space_concatenate(&committed_snap, &draft_tail)`, исключая стирание ранее сказанных фраз. Фиксация в `committed_text` происходит при паузах речи или hard cutoff (12–15 с).
  4. **Сохранение защит финализации**: вся цепочка `trim_silence`, Acoustic Guard (350 мс / RMS 0.003), отсечение галлюцинаций («Спасибо», титры) и LLM-форматтер на кнопке «Стоп» полностью сохранены.
  5. **Фронтенд сплит (`HeaderBar.tsx`)**: `splitCommittedAndDraft` делит отображаемый текст: стабильный префикс белым цветом, последние 1–2 слова — серым курсивом с пульсирующей красной точкой-курсором.
- Все проверки пройдены: `cargo check`, `cargo clippy -- -D warnings` (0 warnings), `cargo fmt --check`, `cargo test` (113 pass), `bun run test` (68 pass), `bun run lint` (0 errors).
...
- **Сессия 16 (Архитектурный апгрейд Real-Time транскрибации по стандарту Google Live Transcribe):**
  ...
  - Внедрена защита от очередей `is_inflight` (backpressure) и ускорение опроса до 350мс.
  - Обновлены системные промпты форматирования (`REFINEMENT_SYSTEM_PROMPT`): сохранение смысловых междометий («М-м!», «Ого!») при фильтрации заминок («эээ», «нуу»).
  - Внедрен UI-сплит `splitCommittedAndDraft` в `HeaderBar.tsx` и `text.ts`.
  - 98 Rust unit tests pass, 65 Vitest frontend tests pass, clippy 0 warnings, eslint 0 errors.
...
- [2026-09-04] **Сессия модернизации README, Лендинга и визуализации пайплайна (v1.4.0)**:
  - **Подпись автора**: Везде удалено «Alexios Odos», подпись приведена к каноническому виду из `avpdev.com`: `Aliaksei Patskevich (AVPDev)` / `Алексей Пацкевич (AVPDev)`, роль `AI Solutions Architect` (обновлены `README.md`, `README.ru.md`, `docs/index.html`, `docs/AI_PROMPTS.md`, `docs/CHANGELOG.md`, `docs/CHANGELOG.ru.md`, `docs/TECHNICAL.md`, `docs/TECHNICAL.ru.md`).
