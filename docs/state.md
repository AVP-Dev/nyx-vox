...
## Версии (важно)
- Последний ПУБЛИЧНЫЙ релиз на GitHub — **v1.3.1** (STT Reliability Guarantee, Keep-Alive Pooling & Offline Whisper Turbo Optimizations)
...
- Реализованы и протестированы ключевые фичи и исправления:
  1. **100% надёжность распознавания (устранена потеря текста)**:
     - `stop_recording` всегда запускает авторитетную батч-транскрипцию всего записанного аудио-буфера (`deepgram`, `whisper`, `groq`, `gemini`, `gigachat`).
     - Исключена потеря окончаний фраз и обрезание текста из-за сетевых лагов стриминга.
     - `streamed_text` сохранён как безопасный fallback только при пустом ответе батч-STT.
  2. **Защита от 429 Rate Limits и зависаний бегущей строки**:
     - Увеличены таймауты HTTP клиентов превью до 4000 мс.
     - Интервал опроса оптимизирован до 1500 мс во всех фоновых воркерах (`ai_provider.rs`, `whisper/recording.rs`, `deepgram.rs`).
  3. **Максимальное ускорение офлайн Whisper (Metal + CoreML + Prewarm)**:
     ...
     - Уменьшение audio tail padding с 500 мс до 200 мс.
  4. **Адаптивный VAD (Silence Auto-Stop)**:
  ...
  5. **Накопительный буфер в WebSocket стриминге (Deepgram)**:
     - Аккумуляция подтверждённых предложений (`accumulated`) с дописыванием промежуточных слов (`live`).
  6. **Индикация копирования и оптимизация истории**: мемоизация `HistoryCard`, мгновенный поиск, Escape-навигация.
