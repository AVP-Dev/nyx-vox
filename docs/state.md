...
- Реализованы и протестированы ключевые фичи и исправления:
  1. **Быстрый живой стриминг бегущей строки (Live Streaming Ticker)**:
     - Восстановлен легковесный фоновый стриминг-воркер (интервал 800 мс с VAD-проверкой RMS) для отображения живой бегущей строки во время диктовки.
     - Для Deepgram: нативный WebSocket-стриминг.
  2. **100% сохранение всех авторских слов и точная расстановка пунктуации (Verbatim & Few-Shot)**:
     - Добавлено строгое правило расстановки запятых при перечислении чисел, слов и однородных членов (например: «один два три четыре...» -> «один, два, три, четыре...»).
     - Добавлен Few-Shot пример перечисления чисел в системный промпт форматирования.
  3. **Разделение промптов для Whisper ASR и Мультимодальных LLM (Gemini / GigaChat)**:
     - Исключена передача текстовых описаний проекта в Gemini / GigaChat, предотвращая переключение моделей в режим чат-бота.
