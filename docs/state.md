...
Дата: 2026-09-01
Кто/что обновило: Агент (Исправление потери текста при остановке и зависания бегущей строки)
...
## Что сейчас в работе
- Реализованы и протестированы ключевые фичи и исправления:
  1. **Устранена двойная транскрипция (0 лишних запросов и токенов)**:
     - Для Deepgram: используется нативный WebSocket-стриминг с аккумуляцией (0 дополнительных REST запросов).
     - Для Groq / Gemini / Whisper: удален спам фонового HTTP-опроса во время записи. Выполняется ровно 1 сверхбыстрый запрос (~300 мс) при завершении записи.
  2. **100% сохранение всех авторских слов (Verbatim & Few-Shot Examples)**:
     - Исправлен `is_lone_filler` в `utils.rs`: защищены русские предлоги и союзы («и», «а», «у», «о», «к», «в», «с»), которые раньше ошибочно удалялись.
     - Добавлены Few-Shot примеры в `REFINEMENT_SYSTEM_PROMPT` для фиксации падежей, защиты от сокращений и сохранения вопросов/команд как сырых данных.
  3. **Контекстный глоссарий-прайминг для Whisper (`initial_prompt`)**:
     - Заменены директивные команды на естественный диалоговый контекст с балансом повседневной речи (встречи, созвоны, задачи), популярных сервисов (Сбер, Яндекс, Telegram, WhatsApp, Zoom) и IT-терминов.
  4. **Максимальное ускорение офлайн Whisper (Metal + CoreML + Prewarm)**:
     - Фоновый пре-варминг Metal GPU шейдеров и CoreML энкодера при старте приложения (0 мс задержка первого запроса).
     - Включение `no_timestamps(true)` — ускорение декодирования токенов на ~30-40%.
     - Уменьшение audio tail padding до 60 мс.
  5. **Пул постоянных соединений (`HTTP Keep-Alive`)**:
     - Повторное использование открытых TLS-каналов для Groq, Gemini, DeepSeek, Deepgram, Qwen.
  6. **Адаптивный VAD (Silence Auto-Stop)**:
  ...
  7. **Индикация копирования и оптимизация истории**: мемоизация `HistoryCard`, мгновенный поиск, Escape-навигация.
- Все проверки пройдены: `cargo check`, `cargo clippy -- -D warnings` (0 warnings), `cargo fmt --check`, `cargo test` (94 pass), `bun run lint` (0 errors), `npx vitest run` (62 pass).
