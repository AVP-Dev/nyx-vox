...
Дата: 2026-08-05
Кто/что обновило: Командный агент (фиксы Enter/компактного режима + релизная документация)
...
- Текущая разработка идёт в ветке `dev` (11 коммитов впереди `main`: GigaChat, P1, тесты, аудит, багфиксы UI)
...
8. **Пользовательский словарь** — свои термины, имена, техническая лексика
9. **Детекция активности речи (VAD)** — автопауза записи при тишине

### 🔵 Архитектура / платформы
10. **Кроссплатформенность (Linux)** — Tauri кроссплатформенный, но бэкенд привязан к macOS (objc2, core-graphics, accessibility-client) — нужен рефакторинг под абстракцию платформы
11. **Error boundary** (frontend) — fallback UI при React crash (технический долг)
12. **Structured logging** (frontend) — tauri-plugin-log (технический долг)
13. **CI/CD pipeline** — GitHub Actions: clippy + test + build при PR (технический долг)
