...
## Универсальный многоуровневый Fallback и отказоустойчивость

Архитектура NYX Vox построена по принципу **движко-независимой (Engine-Agnostic) отказоустойчивости**: не имеет значения, какой именно сервис выбран в настройках (GigaChat, Groq, Whisper, Gemini, Deepgram, DeepSeek, Qwen) — при наличии активного ключа цепочка гарантирует сохранность 100% надиктованного текста:

```
[Остановка: Стоп / VAD]
       │
       ▼
[Проверка плотности стриминга]
   ├── Плотность в норме (>0.7 слов/сек) ──> Мгновенный Single-Pass (0 сек)
   └── Зависание/Лимит стриминга ─────────> Фоллбэк 1: Полный батч-STT аудиофайла
                                                         │
                                                         ▼
                                             [AI-форматирование LLM]
                                                ├── 200 OK ────────> Форматированный текст
                                                ├── 429/503 Лимит ─> Авто-повтор Retry (1.2с)
                                                └── Ошибка/Таймаут ─> Фоллбэк 2: Сохранение исходного сырого STT
                                                                           │
                                                                           ▼
                                                                  [Авто-вставка Cmd+V]
```

1. **Фоллбэк 1 (Уровень STT)**: Если стриминг замер на середине длинной фразы (например, из-за сетевого спайка или rate limit), система сравнивает длительность аудио и число распознанных слов. При нехватке слов автоматически запускается полный батч-STT на записанном аудиофайле.
2. **Фоллбэк 2 (Уровень AI)**: Если модель форматирования не отвечает или исчерпала лимиты квот (429/503), после попытки авто-повтора система **никогда не сбрасывает надиктованный текст**, а мгновенно передает на вставку исходный результат распознавания.
3. **Фоллбэк 3 (Уровень Вставки)**: При отсутствии системных прав Accessibility окно не закрывается впустую, а отображает результат с возможностью скопировать текст в один клик.
