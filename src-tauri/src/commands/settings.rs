...
#[tauri::command]
pub async fn set_formatting_mode(
    app: AppHandle,
    state: State<'_, FormattingMode>,
    mode: String,
) -> Result<(), String> {
    let store = app
        .store("settings.json")
...
#[tauri::command]
pub async fn set_formatting_style(
    app: AppHandle,
    state: State<'_, FormattingStyleState>,
...
#[tauri::command]
pub async fn get_formatting_style(
    state: State<'_, FormattingStyleState>,
) -> Result<FormattingStyle, String> {
    Ok(*state.0.lock().map_err(|e| e.to_string())?)
}

#[tauri::command]
pub async fn set_whisper_model_type(
    model: WhisperModelType,
    state: State<'_, WhisperModel>,
    app: AppHandle,
) -> Result<(), String> {
    let store = app
        .store("settings.json")
...
#[tauri::command]
pub async fn get_whisper_model_type(
    state: State<'_, WhisperModel>,
) -> Result<WhisperModelType, String> {
    Ok(*state.0.lock().map_err(|e| e.to_string())?)
}
...
pub async fn get_api_key(
    ...
    if let Ok(lock) = state.0.lock() {
        if let Some(Some(key)) = lock.get(&service) {
            return Ok(key.clone());
        }
    }
    Ok(String::new())
...
pub async fn get_services_status(
    state: State<'_, keys::ApiKeys>,
) -> Result<HashMap<keys::Service, bool>, String> {
    let mut status = HashMap::new();
    if let Ok(lock) = state.0.lock() {
        for (service, key_opt) in lock.iter() {
            status.insert(
                service.clone(),
                key_opt.as_ref().is_some_and(|k| !k.is_empty()),
            );
        }
    }
    Ok(status)
...
pub async fn get_history_settings(app: AppHandle) -> Result<(bool, String), String> {
    let store = app
        ...
        .map_err(|e: tauri_plugin_store::Error| e.to_string())?;
    let cleanup = store
        .get("history_smart_cleanup")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    let period = store
        .get("history_retention_period")
        .and_then(|v| v.as_str().map(|s| s.to_string()))
