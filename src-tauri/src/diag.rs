...
pub async fn run_self_diagnosis(app: AppHandle) -> Result<serde_json::Value, String> {
    ...
    // 1. Check states
    results.insert(
        "processing_flag".to_string(),
        serde_json::json!(app.try_state::<ProcessingFlag>().is_some()),
    );
    results.insert(
        "recording_flag".to_string(),
        serde_json::json!(app.try_state::<RecordingFlag>().is_some()),
    );
    results.insert(
        "target_app_state".to_string(),
        serde_json::json!(app.try_state::<TargetApp>().is_some()),
    );

    // 2. Check model
    let whisper_model_available =
        if let Some(m_state) = app.try_state::<crate::state::WhisperModel>() {
    ...
    results.insert(
        "whisper_model_available".to_string(),
        serde_json::json!(whisper_model_available),
    );

    // 3. Check App Path
    results.insert(
        "executable_exists".to_string(),
        serde_json::json!(app.path().executable_dir().is_ok()),
    );
