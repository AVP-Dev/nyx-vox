...
pub fn perform_smart_cleanup(app: &AppHandle) -> Result<(), String> {
    ...
    let cleanup_enabled = settings_store
    ...
    let mut history: Vec<HistoryEntry> = history_store
        .get("entries")
        .and_then(|v| serde_json::from_value(v.clone()).ok())
        .unwrap_or_default();
    ...
    if history.len() != initial_len {
        ...
        log::info!(
            "Smart Cleanup removed {} old entries",
            initial_len - final_count
        );
    }
...
mod tests {
    ...
    fn retention_periods_are_monotonically_increasing() {
        ...
        for window in values.windows(2) {
            assert!(
                window[0] < window[1],
                "Periods must be monotonically increasing"
            );
        }
    }
